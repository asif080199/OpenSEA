...
cx_mat GlobalSolution::calcOutput(double freqIn)
{
    ...
    //Get number of rows
    int nrow;
    nrow = getSolution().refSolution().n_rows;

    //resize matrix
    output.resize(1, nrow);

    for(unsigned int i = 0; i < nrow; i++)
	{
        output[1,i] = pow(wavefreq, orderDerivative) * pow(compI, orderDerivative) * getSolution().getSolution(i);
	}
    return output;
}
...
Solution &GlobalSolution::getSolution(double freqIn)
{
    ...
    for (int i = 0; i < pParent.reflistFreq.size(); i++)
    {
        //Check if the frequency matches the specified freq in.
        if (pParent.reflistFreq()[i] == freqIn)
        {
            freqIndex = i;
            break;
        }
    ...
    return pParent.plistSolution[pParent.getCurBodyIndex()].refSolution(getCurWaveInd(), freqIndex);
}
