...
#ifndef DICTHYDROREACT_H
#define DICTHYDROREACT_H
...
//######################################### Class Separator ###########################################################
/**
 * The dictHydroReact class is a template class for the dictionaries that read reactive forces.
 *
 ...
 *
 * The dictHydroReact class is itself derived from the Dictionary class.  The Dictionary class defines most of the
 * behavior.  The only thing added to this class is the behavior for how to handle the individual keywords specific
 * to this input file.  In the case of the dictHydroReact class, the same keywords are used in all related input
 * files, hence why a generic class can be used with inheritence.  The following input files are processed with
 * this class.
 * 1.)  hydrodamp.out
 * 2.)  hydromass.out
 * 3.)  hydrostiffness.out
 * @sa Dictionary
 * @sa FileReader
 * @sa HydroReader
 */
...
protected:
    ...
    static std::string OBJECT_DATA; /**< Datat object designator. */
