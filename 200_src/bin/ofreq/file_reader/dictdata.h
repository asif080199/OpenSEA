 ...
 *---------------------------------------------------------------------------------------------------------------------
 *Sep 1 2014	Nicholas Barczak		Initially Created
 *
\*-------------------------------------------------------------------------------------------------------------------*/
...
//######################################### Class Separator ###########################################################
/**
 * The dictData class defines the key-word value pairs associated with the data.in input file.  Just as a normal
 * dictionary defines the meaning of words, the dictData class works in the same way.  The dictData class takes
 ...
 * the dictData object needs to interact with.  It will use the properties of those objects to apply the values
 * it finds in the key-value pair.  Any objects created in the dictData class can be safely deleted once all file
 * reading is done.
 *
 * Note:  The code for the dictData object always references the last object in the list.  This assumes that no
 ...
 * @sa FileReader
 */
class dictData : public Dictionary
{
    Q_OBJECT
...
public:
     ...
     */
    dictData(osea::FileReader *parent);
...
private:
    //------------------------------------------Function Separator ----------------------------------------------------
    //Class Names
    static std::string OBJECT_HYDROFILE; /**< Keyword for the hydrofiles object.*/

    //------------------------------------------Function Separator ----------------------------------------------------
    //Keyword Names
    static std::string KEY_FILELOCATION; /**< Keyword for the file location list. */
