 ...
 *---------------------------------------------------------------------------------------------------------------------
 *Dec 18, 2014	Nicholas Barczak		Initially Created
...
#ifndef REPWAVESPECTRA_H
#define REPWAVESPECTRA_H
#include "report.h"
...
//######################################### Class Separator ###########################################################
/**
 * @brief Report for body specific wave spectra.
 *
 * Provides the wave spectral energy for each body, for each wave direction.  The following keywords are defined.
 *
 * KEY_BODY:        String.  Name of the body to associate with.
 * KEY_AMP:         String.  Declares if wave amplitude (wave_amp) is calculated, or wave energy (energy)
 */
class repWaveSpectra : public Report
{
//==========================================Section Separator =========================================================
public:
    //------------------------------------------Function Separator ----------------------------------------------------
     ...
     */
    repWaveSpectra(ofreq::ReportManager *ptIn);
...
private:
    ...
    //------------------------------------------Function Separator ----------------------------------------------------
    bool pCalcAmp = false;  /**< Decides whether to calculate wave amplitude, or wave energy. */

    //------------------------------------------Function Separator ----------------------------------------------------
    //Static variables
    static std::string KEY_BODY;    /**< Name of the body to associate with.*/
    static std::string KEY_WAVE;    /**< String of whether to calculate wave amplitude or spectral energy. */

    static std::string VAL_AMP;     /**< String that declares should calculate wave amplitude. */
    static std::string VAL_ENERGY;  /**< String that declares should calculate wave energy. */
