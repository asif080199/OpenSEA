...
//######################################### Class Separator ###########################################################
/**
 * @brief The Report for calculation of relative efficiency.
 *
 * Relative efficiency is the capture width divided by some body length.  The body length is a user input.  This
 * Report internally calculates the capture with, and then divides by the user input for body length.  It requires
 * all the same inputs as the capture width report.  The user can also specify a list of which variable to include
 * in the summation of power extracted, when calculating efficiency.  The Report requires reference to another
 * Report for power calculation.  The relative efficiency changes, depending on what terms are included in the
 * Power calculation.  The user can specify a list of exactly which terms to include in the summation from the
 * Power calculation.  The following keywords are declared:
 *
 ...
 * KEY_LENGTH:      Double.  The length of the body used to calculate relative efficiency.
 */
class repRelEff : public ofreq::Report
{
//==========================================Section Separator =========================================================
public:
    //------------------------------------------Function Separator ----------------------------------------------------
     ...
     */
    repRelEff(ofreq::ReportManager *ptIn);
...
private:
    ...
    //------------------------------------------Function Separator ----------------------------------------------------
    /**
     * @brief The body length.  User input.  Used to calculate relative efficiency.
     */
    double pLength = 0;
    ...
    static std::string KEY_LENGTH;      /**< The user input for length used to calculate relative efficiency. */
};
